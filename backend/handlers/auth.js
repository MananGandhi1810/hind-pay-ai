...
const registerHandler = async (req, res) => {
    const { name, email, password, vpa, phone } = req.body;
    if (
        !name ||
        !email ||
        !password ||
    ...
    const existingUser = await prisma.user.findFirst({
        where: { OR: [{ email }, { vpa }, { phone }] },
    });
    if (existingUser) {
        return res.status(403).json({
            success: false,
            message: "A user with this email, VPA or phone already exists",
            data: null,
        });
    }
    const hashedPassword = await hash(password);
    var user = await prisma.user.create({
        data: { name, email, vpa, phone, password: hashedPassword },
    });
